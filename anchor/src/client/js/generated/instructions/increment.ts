...
import { SUPPLY_CHAIN_PROGRAM_ADDRESS } from '../programs';
...
export type IncrementInstruction<
  TProgram extends string = typeof SUPPLY_CHAIN_PROGRAM_ADDRESS,
  TAccountSupplyChain extends string | IAccountMeta<string> = string,
  TRemainingAccounts extends readonly IAccountMeta<string>[] = [],
> = IInstruction<TProgram> &
  IInstructionWithData<Uint8Array> &
  IInstructionWithAccounts<
    [
      TAccountSupplyChain extends string
        ? WritableAccount<TAccountSupplyChain>
        : TAccountSupplyChain,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountSupplyChain extends string = string> = {
  supply_chain: Address<TAccountSupplyChain>;
};

export function getIncrementInstruction<
  TAccountSupplyChain extends string,
  TProgramAddress extends Address = typeof SUPPLY_CHAIN_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountSupplyChain>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountSupplyChain> {
  // Program address.
  const programAddress = config?.programAddress ?? SUPPLY_CHAIN_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    supply_chain: { value: input.supply_chain ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.supply_chain)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountSupplyChain>;
